...
const ResizableMedia: React.FC<ResizableMediaProps> = ({ node, updateAttributes, selected }) => {
  const { src, alt, title, width = 300, height = 200 } = node.attrs;
  const [dimensions, setDimensions] = useState({ width, height });
  const [aspectRatio, setAspectRatio] = useState(width / height);
  const containerRef = useRef<HTMLDivElement>(null);
  const isYouTube = node.attrs['data-youtube-video'];
  ...
  const renderContent = () => {
    if (isYouTube) {
      ...
      const videoId = node.attrs['data-youtube-video'];
      return (
        <iframe
